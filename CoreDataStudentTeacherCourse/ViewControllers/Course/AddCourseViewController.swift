...
class AddCourseViewController: UIViewController {
    ...
    let courseNames = ["iOS", "Android", "UI/UX"]
    ...
    func emptyFieldsValidationAlert() {
        ...
        let alert = UIAlertController(title: validationAlertTitle, message: validationAlertMessage, preferredStyle: UIAlertControllerStyle.alert)
        let action = UIAlertAction(title: okTitle, style: UIAlertActionStyle.default)
    ...
    @IBAction func saveCourse(_ sender: UIButton) {
        ...
        if courseDetailsField.text == "" {
            ...
            if let _ = context {
                ...
                course.courseId = Utility.generateUUID()

                do {
                    try context?.save()
                    courseDetailsField.text = ""
                } catch let error {
